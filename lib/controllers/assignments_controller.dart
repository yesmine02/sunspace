...
class AssignmentsController extends GetxController {
  ...
  var searchQuery = ''.obs;
  var isManagementMode =
      false.obs; // ✅ Nouveau : Mode gestion pour Enseignant/Admin
  ...
  Future<void> fetchAssignments() async {
    try {
      ...
      if (isManagementMode.value && _authController.isAdmin) {
        url =
            '$_baseUrl?populate[submissions][populate][student]=true&populate[submissions][populate][file]=true&populate[attachment]=true&populate[course]=true&sort[0]=createdAt:desc';
      } else if (_authController.isInstructor && user != null) {
        url =
            '$_baseUrl?populate[course][populate][instructor]=true&populate[submissions][populate][student]=true&populate[submissions][populate][file]=true&populate[attachment]=true&filters[course][instructor][id][\$eq]=${user['id']}&sort[0]=createdAt:desc';
      } else if ((_authController.isStudent || _authController.isAdmin) &&
          user != null) {
        ...
        url =
            '$_baseUrl?populate[course]=true&populate[attachment]=true&populate[submissions][populate][student]=true&populate[submissions][populate][file]=true&pagination[pageSize]=100&sort[0]=createdAt:desc';
      } else {
        url =
            '$_baseUrl?populate=course&populate=submissions&populate=attachment&sort[0]=createdAt:desc';
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (data['data'] is List) {
          ...
          if ((_authController.isStudent || _authController.isAdmin) &&
              !isManagementMode.value) {
            ...
            if (enrolledIds.isNotEmpty || enrolledDocIds.isNotEmpty) {
              loadedAssignments = loadedAssignments.where((a) {
                ...
                final isByDocId =
                    a.courseDocumentId != null &&
                    enrolledDocIds.contains(a.courseDocumentId);
  ...
  Future<void> addAssignment(
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (file != null) {
          final createdId =
              createdData['data']?['id']; //On récupère l’ID du devoir créé.
          if (createdId != null) {
        ...
        try {
          final instructorName =
              _authController.currentUser.value?['username'] ?? 'Un enseignant';
          final notifCtrl = Get.find<NotificationController>();
          final createdId =
              createdData['data']?['id'] ?? createdData['data']?['documentId'];

          notifCtrl.notifyMembers(
            title: 'Nouveau devoir publié !',
            message:
                '$instructorName a publié un nouveau devoir : "${assignment.title}"',
            type: 'Info',
  ...
  Future<void> updateAssignment(Assignment assignment, dynamic courseId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Get.snackbar(
          'Erreur',
          'Échec de la modification (${response.statusCode})',
        );
      }
    } catch (e) {
  ...
  Future<void> deleteAssignment(String documentId) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        ...
        Get.snackbar(
          'Erreur',
          'Impossible de supprimer (${response.statusCode})',
        );
      }
    } catch (e) {
        ...
        try {
          ...
          if (assignment != null && assignment.courseId != null) {
            ...
            final course = coursesCtrl.courses.firstWhereOrNull(
              (c) => c.id.toString() == assignment.courseId,
            );
            ...
            if (course != null && course.instructorId != null) {
              ...
              await notifCtrl.sendNotification(
                ...
                message:
                    '$studentName a soumis son travail pour le devoir "${assignment.title}".',
                type: 'Info',
                relatedType: 'submission',
                relatedId: submissionId,
                actionUrl:
                    '/dashboard/instructor/assignments', // Lien vers la gestion des devoirs
              );
            }
          }
        } catch (e) {
          debugPrint(
            'Erreur lors de l\'envoi de la notification de soumission: $e',
          );
        }
  ...
  Future<void> _uploadSubmissionFile(
    dynamic entryId,
    PlatformFile file,
    String token,
  ) async {
    try {
      ...
      request.fields['field'] =
          'file'; // Correction : s'appelle 'file' selon votre schéma
  ...
  Future<void> _loadFromCache() async {
    ...
    if (prefs.containsKey('cached_assignments')) {
      final Map<String, dynamic> responseData = jsonDecode(
        prefs.getString('cached_assignments')!,
      );
      final List<dynamic> data = responseData['data'] ?? [];

      List<Assignment> loaded = data
